...
modules as objects for this topic.
...
`list` with the function `foo` as only element. Since modules are isolated from
the `.GlobalEnv` the following object `x` can not be found:
