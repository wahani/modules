...
data analysis projects can span a thounsand lines easily. R has two important
...
(refering to the function `source`) them in order. Modules, in this context,
present a more sophisticated way to *source* files by providing three important
features:

- (Imports) loading a package is local to a module and avoids name clashes in
the global environment.
- (Exports) variable assignment are local to a module and (a) do not polute the
global environment and (b) hide details of a module.
- Modules make it easy to spread your code base accross files and reuse them
...
*connects the dots*. `munging.R` and `graphics.R` implement helper functions but
do not execute any of the analysis.
...
are safe to use and avoids that other parts of our code base relies on
